...
# Prefer an explicitly configured binary or one on PATH, then use the
# per-user installation location used by the datamover account.
if ! command -v "$ILLUMINATE_BIN" >/dev/null 2>&1; then
    ILLUMINATE_BIN="/home/${USER:-$(id -un)}/bin/illuminate"
fi
